...
// The array contains NSStrings. These strings corrospond to the string constants used by Apple.
@property (unsafe_unretained, nonatomic, readonly) NSArray *excludedAppleActivityTypes;
...
// Notification that the end user attempted to copy data to the pasteboard when copying to the pasteboard is unauthorized.
//
// The pasteboard policy for AppConnect apps has these possible status values:
// 1. ACPASTEBOARDPOLICY_UNAUTHORIZED: Writing data to the pasteboard is not allowed. The end user is displayed the "Copy" button, but tapping it has no effect. This behavior can confuse the end user, who sometimes thinks that the selection was copied.
// 2. ACPASTEBOARDPOLICY_AUTHORIZED: Writing data to the pasteboard is allowed and the data can be shared with all other apps.
// 3. ACPASTEBOARDPOLICY_SECURECOPY: Writing data to the pasteboard is allowed, but the data can be pasted only into other AppConnect apps.
//
// This notification is sent only when the status is ACPASTEBOARDPOLICY_UNAUTHORIZED. The notification tells the app that the end user attempted to copy data. The app is responsible for taking some action, such as displaying an alert to the end user that copy is not allowed.
